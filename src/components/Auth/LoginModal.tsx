import React from 'react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
} from '@/components/ui/dialog';
import { authClient } from '@/lib/auth-client';

interface LoginModalProps {
  open: boolean;
  onClose: () => void;
}

interface Provider {
  id: 'google' | 'github' | 'kakao' | 'naver';
  label: string;
  icon: React.ReactNode;
  className: string;
}

// 단색 SVG 아이콘
const GoogleIcon = () => (
  <svg viewBox="0 0 24 24" className="size-5 fill-current" aria-hidden="true">
    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
  </svg>
);

const GitHubIcon = () => (
  <svg viewBox="0 0 24 24" className="size-5 fill-current" aria-hidden="true">
    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
  </svg>
);

const KakaoIcon = () => (
  <svg viewBox="0 0 24 24" className="size-5 fill-current" aria-hidden="true">
    <path d="M12 3C6.48 3 2 6.48 2 10.8c0 2.73 1.6 5.14 4.02 6.58L4.9 21l4.36-2.28c.88.17 1.8.28 2.74.28 5.52 0 10-3.48 10-7.8S17.52 3 12 3z" />
  </svg>
);

const NaverIcon = () => (
  <svg viewBox="0 0 24 24" className="size-5 fill-current" aria-hidden="true">
    <path d="M16.273 12.845L7.376 0H0v24h7.727V11.155L16.624 24H24V0h-7.727z" />
  </svg>
);

const PROVIDERS: Provider[] = [
  {
    id: 'google',
    label: 'Google로 로그인',
    icon: <GoogleIcon />,
    className: 'bg-white text-gray-800 border border-gray-300 hover:bg-gray-50',
  },
  {
    id: 'naver',
    label: 'Naver로 로그인',
    icon: <NaverIcon />,
    className: 'bg-[#03c75a] text-white hover:bg-[#02b350]',
  },
  {
    id: 'kakao',
    label: 'Kakao로 로그인',
    icon: <KakaoIcon />,
    className: 'bg-[#fee500] text-[#191919] hover:bg-[#f5da00]',
  },
  {
    id: 'github',
    label: 'GitHub로 로그인',
    icon: <GitHubIcon />,
    className: 'bg-gray-900 text-white hover:bg-gray-700',
  },
];

export default function LoginModal({ open, onClose }: LoginModalProps) {
  const handleLogin = async (provider: Provider['id']) => {
    await authClient.signIn.social({
      provider,
      callbackURL: window.location.href,
    });
  };

  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader className="items-center gap-3">
          <div className="text-2xl font-bold tracking-tight">DevLog</div>
          <DialogTitle className="text-base font-medium text-muted-foreground">
            SSO Login
          </DialogTitle>
        </DialogHeader>

        <div className="flex flex-col gap-3 py-2">
          {PROVIDERS.map((p) => (
            <button
              key={p.id}
              onClick={() => handleLogin(p.id)}
              className={`flex w-full items-center gap-3 rounded-md px-4 py-2.5 text-sm font-medium transition-colors ${p.className}`}
            >
              {p.icon}
              <span className="flex-1 text-center">{p.label}</span>
            </button>
          ))}
        </div>

        <p className="text-center text-xs text-muted-foreground leading-relaxed">
          로그인 시 이메일에 대한 정보만 수집합니다.
          <br />
          가입 탈퇴 시 이메일 정보는 자동 삭제됩니다.
        </p>
      </DialogContent>
    </Dialog>
  );
}
